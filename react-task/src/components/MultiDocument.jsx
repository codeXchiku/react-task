...
const MultiDocument = ({
  ...
  return (
  <div className="max-w-2xl mx-auto mt-5 p-4 sm:p-6">
      ...
      <div className="text-center">
        ...
        <div className="mt-3 flex flex-col sm:flex-row items-center gap-3 justify-center">
          <button
            ...
            text-sm font-medium hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            Select files
          </button>
    ...
    {error && (
      <div
        className="mt-3 text-sm text-red-600 whitespace-pre-line break-words"
        role="status"
      >
        {error}
      </div>
    ...
    <div className="mt-4">
      <div className="space-y-3">
        {files.map((f) => (
          <div
            key={f.id}
            className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-3 border rounded-md bg-gray-50"
          >
            {/* Preview */}
            <div className="w-20 h-20 sm:w-14 sm:h-14 flex-shrink-0 rounded-md overflow-hidden bg-white border">
              {f.preview ? (
                <img
                  src={f.preview}
                  alt={f.file.name}
    ...
    <div className="mt-4 flex flex-col sm:flex-row gap-2 sm:items-center">
      ...
      <button
        ...
        className="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md shadow-sm 
      ...
      <button
        ...
        hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-200"
      >
        Clear
      </button>

      <div className="text-xs text-gray-400 sm:ml-auto mt-1 sm:mt-0">
        Max per file: {Math.round(maxSizeBytes / 1024 / 1024)}MB
      </div>
    </div>
  </div>
);
